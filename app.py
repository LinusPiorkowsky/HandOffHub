...
@login_required
def handoffs():
    """View all handoffs for the current user's team"""
    if not current_user.team_id:
        flash('You need to be part of a team to view handoffs.', 'warning')
        return redirect(url_for('dashboard'))
    ...
    if filter_type == 'received':
        query = Handoff.query.filter_by(to_team_id=current_user.team_id)
    elif filter_type == 'sent':
        query = Handoff.query.filter_by(from_team_id=current_user.team_id)
    else:  # all
        query = Handoff.query.filter(
            or_(
    ...
    handoffs = query.order_by(Handoff.created_at.desc()).paginate(
...
def view_handoff(handoff_id):
    ...
    return render_template('view_handoffs.html',
                         handoff=handoff,
                         form=form,
                         comments=comments,
...
def view_handoff(handoff_id):
    ...
    # WICHTIG: Der Template-Name muss mit dem Dateinamen übereinstimmen!
    # Benenne entweder die Datei von view_handoffs.html zu view_handoff.html um
    # ODER ändere hier zu 'view_handoffs.html'
    return render_template('view_handoff.html',  # <-- Stelle sicher, dass diese Datei existiert!
                         handoff=handoff,
                         form=form,
                         comments=comments,
                         sub_handoffs=sub_handoffs,
...
with app.app_context():
    try:
        # Call init_database which will create tables and demo data
        init_database()
