...
@app.route('/init-database-now-delete-this')
def init_database_temp():
    ...
    try:
        ...
        if Organization.query.first():
            return "<h1>Database already initialized!</h1>"
        ...
        team = Team(name="Admin Team", organization_id=org.id, color="#6366f1")
        db.session.add(team)
        db.session.flush()
        ...
        <head>
            ...
            <style>
                ...
                .success { color: green; }
                .warning { color: red; background: yellow; padding: 10px; }
        ...
        <body>
            ...
            <div class="info">
                ...
                <p><strong>Password:</strong> ChangeThisPassword123!</p>
            </div>
            <div class="warning">
                <h2>⚠️ IMPORTANT - DELETE THIS ROUTE NOW!</h2>
                <p>1. Go back to GitHub</p>
                <p>2. Edit app.py</p>
                <p>3. Remove the /init-database-now-delete-this route</p>
                <p>4. Commit the change</p>
            </div>
            <p><a href="/">Go to Homepage</a> | <a href="/login">Go to Login</a></p>
        </body>
        </html>
        """
    except Exception as e:
        return f"<h1>Error: {str(e)}</h1><p>Check Railway logs for details.</p>"
